 ...
 *      Author: Joe Krachey
 *      Edited by: Aaron Cohen; Arian Noori
 */
...
void Crystalfontz128x128_SetDrawFrame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    HAL_LCD_writeData( (x0 ) >> 8); // Shift MSBs right by 8
    HAL_LCD_writeData(x0);
    HAL_LCD_writeData( (x1) >> 8); // Shift MSBs right by 8
    ...
    HAL_LCD_writeData( (y0 ) >> 8); // Shift MSBs right by 8
    HAL_LCD_writeData(y0 );
    HAL_LCD_writeData( (y1 ) >> 8); // Shift MSBs right by 8
...
/**
 * Reads in lux and displays splash screen in lighting-appropriate colors
 * Intentionally independent from similar FreeRTOS task so this can be displayed before
 * FreeRTOS scheduler used
 */
const uint8_t* images[] = {&ouroborousBitmaps, &controlsBitmaps, &lossBitmaps, &play_againBitmaps};
void lcd_splash_screen(enum image image){
    float lux = opt3001_read_lux();
    const int colorfg = (lux < 500) ? OUROBOROUS_BEIGE : OUROBOROUS_BLUE;
    const int colorbg = (lux < 500) ? LCD_COLOR_BLACK : OUROBOROUS_BEIGE;
    lcd_draw_image(
          LCD_VERTICAL_MAX/2 - 2,
